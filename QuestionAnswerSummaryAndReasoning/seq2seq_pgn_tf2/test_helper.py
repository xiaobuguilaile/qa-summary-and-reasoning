...
import numpy as np
from QuestionAnswerSummaryAndReasoning.seq2seq_pgn_tf2.batcher import output_to_words
...
class Hypothesis:
    ...
    def __init__(self, tokens, log_probs, state, attn_dists, p_gens, coverage):
        ...
        self.log_probs = log_probs   # list of the log probabilities of the tokens
    ...
    def extend(self, token, log_prob, state, attn_dist, p_gen, coverage):
        ...
        return Hypothesis(tokens=self.tokens + [token],  # we add the decoded token
                          ...
                          attn_dists=self.attn_dists + [attn_dist],  # we  add the attention dist of the decoded toke
...
def beam_decode(model, batch, vocab, params):
    ...
    enc_outputs, state = model.call_encoder(enc_input)  # enc_outputs shape=(3, 115, 256), state shape=(3, 256),
